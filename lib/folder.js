...
async function getLastModifiedTime (folderPath) {
	let latestDate
	...
	for (const file of files) {
		...
		const date = new Date(
			stats.isDirectory()
				? await getLastModifiedTime(filePath)
				: stats.mtime,
		)
	...
	if (!latestDate) {
		...
		latestDate = new Date(stats.mtime)
	}
...
module.exports = {
	...
	async get () {
		...
		try {
			...
			const time = await getLastModifiedTime(folderPath)
